...
// Goods massivini cixardin sehifeye ve her malin alatinda bu duyme . Duymeye klikleyende mal yazilmalidir local storage. Bir mallar sehifesi  mallari local storegden gosterin
let list = document.getElementById('list')
function PrintAllProd(goods = []) {
    goods.forEach(element => {
        ...
        list.append(myTag)
        list.append(myButton)
        myButton.textContent = 'Add Cart'
        myButton.addEventListener('click', (ev) => {
            ...
            let formDataArray = [];
            let savedUser = localStorage.getItem('Data');
            if (savedUser === null) {
                formDataArray = [];
            } else {
                formDataArray = JSON.parse(savedUser);
            }
            formDataArray.push(product);
...
let btnOpen = document.getElementById('products')
cartBtn.addEventListener('click', (ev) => {
    btnOpen.disabled = false
    document.getElementById('listForLS').style.display = 'inline-block'
    btnOpen.style.display='block'
    cartBtn.style.display = 'none'
    list.innerHTML = ''; 
    let prod = JSON.parse(localStorage.getItem('Data'))
    let lSList = document.getElementById('listForLS')
    lSList.innerHTML = '';

    if(prod.length < 1){
        lSList.innerHTML = '<p>No Product In Cart</p>'
    }
...
btnOpen.addEventListener('click' , (e) => {
    ...
    document.getElementById('listForLS').style.display = 'none'
    PrintAllProd(goods)
})
